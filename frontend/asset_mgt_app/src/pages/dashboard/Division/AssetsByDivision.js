...
const AssetsByDivision = () => {
  ...
  return (
    ...
    <div className="assets-container">
      <h2 className="assets-title">Assets available in {divisionName} Division</h2>
            ...
            {filteredAssets.length > 0 ? (
              filteredAssets.map(asset => (
                <tr key={asset.id}>
                  ...
                  <td>
                    {asset.image ? (
                      <button onClick={() => openModal(`${baseURL}${asset.image}`)} className="view-image-btn">
                        <FaEye /> View Image
                      </button>
                    ) : (
                      <span>No Image</span>
      ...
      {isAdmin && (
        <div className="add-asset-btn-container">
          <Link to={`/add-asset/${id}`} className="add-asset-btn">Add Asset</Link>
        </div>
      )}
    </div>
    </div>
